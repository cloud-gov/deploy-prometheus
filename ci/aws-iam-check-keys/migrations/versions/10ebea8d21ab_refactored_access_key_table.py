...
def upgrade() -> None:
    ...
    for iam_key in session.query(IAMKeys):
        access_key1 = AccessKey()
...
def downgrade() -> None:
    ...
    bind = op.get_bind()
    session=orm.Session(bind=bind)
    for access_key in session.query(AccessKey):
        query = select(IAMKeys).where(IAMKeys.id == access_key.user_id)
